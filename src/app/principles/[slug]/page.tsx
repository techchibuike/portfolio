...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const doc = getMarkdownDocument("principles", `${slug}.md`);
...
export async function generateStaticParams() {
  const posts = getAllMarkdownDocuments("principles");
  return posts.map((post) => ({
    slug: post.slug,
  }));
}

export default async function PrinciplePost({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const doc = getMarkdownDocument("principles", `${slug}.md`);
  ...
  return (
    <div className="container max-w-3xl mx-auto px-4 py-20 space-y-12 animate-fade-in-up">
      <Link href="/principles" className="text-sm text-muted-foreground hover:text-foreground transition-colors mb-12 block">
        &lt; Back to Principles
      </Link>
