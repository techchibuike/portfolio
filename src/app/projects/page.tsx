...
export default function ProjectsPage() {
  const documents = getAllMarkdownDocuments("projects");
  ...
  return (
    <div className="container max-w-3xl mx-auto px-4 py-20 space-y-12 animate-fade-in-up">
      <header className="space-y-4">
        ...
        <p className="text-lg text-muted-foreground">How I solve problems and what I am building.</p>
      </header>
      
      <div className="grid gap-6">
        {documents.map((doc) => (
          <Link key={doc.slug} href={`/projects/${doc.slug}`} className="group block p-6 rounded-lg border border-border/40 bg-card hover:border-border transition-colors">
